...
{
	/// <summary>
	/// Keeps one <see cref="DigitizerState"/> per browser session, keyed by the ASP.NET Core
	/// session cookie rather than by Blazor's SignalR circuit id. A circuit can drop and reconnect
	/// (or fail to reconnect and force a full page reload) independently of this cache, so a
	/// backgrounded tab or a flaky connection never wipes an in-progress digitization the way
	/// storing state in circuit-scoped services would.
	///
	/// Duplicated from <c>PlotDigitizer.Web.Services.DigitizerStateManager</c> on purpose; see
	/// <see cref="DigitizerSessionOptions"/>.
	/// </summary>
