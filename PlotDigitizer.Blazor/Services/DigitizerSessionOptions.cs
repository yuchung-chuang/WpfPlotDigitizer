...
namespace PlotDigitizer.Blazor.Services
{
	...
	/// full size images, so both the idle timeout and the session cap matter.
	///
	/// Duplicated from <c>PlotDigitizer.Web.Services.DigitizerSessionOptions</c> on purpose: this
	/// project is a disposable spike that must never reference the Razor Pages project, and vice
	/// versa. See the Blazor spike decisions captured for this session for why.
	/// </summary>
