...
2. **Follow the established FlaUI shape** from `UiTest.cs`: `Application.Launch("PlotDigitizer.exe")`, `WaitWhileMainHandleIsMissing()`, a `UIA3Automation` instance, and a `[TestCleanup]` that closes and disposes both.
...
6. **Place tests** in `PlotDigitizer.Test/Ui/` with `[TestCategory("UI")]` so the fast suite can exclude them, then run:
