...
namespace PlotDigitizer.Core
{
    public class AxisPageViewModel : ViewModelBase
    {
        ...
        {
            base.Enter();
            logger?.LogInformation("Entered AxisPageViewModel with IsEnabled status: {IsEnabled}", IsEnabled);
            ...
            if (setting.AxisLocation == default) {
                ...
                logger?.LogInformation("Loaded stored axis location from settings: {AxisLocation}", AxisLocation);
            }
        }
        ...
        {
            base.Leave();
            logger?.LogInformation("Leaving AxisPageViewModel with IsEnabled status: {IsEnabled}", IsEnabled);
            ...
            logger?.LogInformation("Stored current axis location to settings: {AxisLocation}", AxisLocation);
        }

        private bool CanGetAxis()
        {
            logger?.LogDebug("CanGetAxis evaluated. IsEnabled status: {IsEnabled}", IsEnabled);
            return IsEnabled;
        }
        ...
        private void GetAxis()
        {
            ...
            try {
                ...
                logger?.LogInformation("Axis location detected successfully: {AxisLocation}", AxisLocation);
            }
            catch (Exception ex) {
                ...
                logger?.LogWarning("Using default axis location: {AxisLocation}", AxisLocation);
            }
        }
